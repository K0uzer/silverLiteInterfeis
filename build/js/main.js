...
function getDocumentsFromFolder(event, level) {
  let parentFolter;
  if (level !== 0) parentFolter = folderThree.filter(item => item.numberAgreement === event.target.parentNode.children[2].textContent);else parentFolter = folderThree.filter(item => item.numberAgreement === event.target.parentNode.children[2].textContent);
